...
#define LIS3DH_MULTI						0x80				// Sub[7] = 1
...
int I2C1_Send3(uint8_t slaveAddr, uint8_t data1, uint8_t data2, uint8_t data3){
  ...
  I2C1_MCS_R = (I2C_MCS_STOP | I2C_MCS_RUN); // master enable
  while (I2C1_MCS_R & I2C_MCS_BUSY){};      // wait for transmission done
  // return check error bits
    return (I2C1_MCS_R & (I2C_MCS_DATACK|I2C_MCS_ADRACK|I2C_MCS_ERROR));
  }
...
int I2C1_RecvTest(uint8_t slaveAddr, uint8_t subAddress ,uint8_t *data){
	...
	//SAD+W	
	I2C1_MSA_R = (slaveAddr << 1) & I2C_MSA_SA_M;      // MSA[7:1] is slave address
  I2C1_MSA_R &= ~I2C_MSA_RS; 									//MSA[0] to write
		
//Sending subaddress before reading
	I2C1_MDR_R = subAddress;
  I2C1_MCS_R = I2C_MCS_START | I2C_MCS_RUN;         // master enable
   while (I2C1_MCS_R & I2C_MCS_BUSY){};            // wait for transmission done
   // check error bits
   if((I2C1_MCS_R & (I2C_MCS_DATACK|I2C_MCS_ADRACK|I2C_MCS_ERROR)) != 0){
			I2C1_MCS_R = I2C_MCS_STOP;                    // stop transmission
	 ...
	 //Repeated Start Condition + SAD+R
	 I2C1_MSA_R = (slaveAddr << 1) | I2C_MSA_RS; //MSA[7:1] is slave address
																							 //MSA[0] to read		
// first              
  I2C1_MCS_R = (I2C_MCS_STOP   |               // positive data ack
                I2C_MCS_START |               // generate start/restart
                I2C_MCS_RUN);                 // master enable
  while (I2C1_MCS_R & I2C_MCS_BUSY) {};       // wait for transmission done
  data[0] = I2C1_MDR_R ;                      // most significant byte               
...
//USE ONLY FOR LIS3DH TO RECIEVE ACCELERATION/6 REGISTERS IN A ROW!
...
//Sending subaddress before reading
	I2C1_MDR_R = subAddress | LIS3DH_MULTI;
  I2C1_MCS_R = I2C_MCS_START | I2C_MCS_RUN;         // master enable
   while (I2C1_MCS_R & I2C_MCS_BUSY){};            // wait for transmission done
   // check error bits
	uint8_t error = I2C1_MCS_R & 0xFF;
   if((I2C1_MCS_R & (I2C_MCS_DATACK|I2C_MCS_ADRACK|I2C_MCS_ERROR)) != 0){
			I2C1_MCS_R = I2C_MCS_STOP;                    // stop transmission
			// return error bits if nonzero
      return (I2C1_MCS_R & (I2C_MCS_DATACK|I2C_MCS_ADRACK|I2C_MCS_ERROR));
...
// third
	I2C1_MCS_R = (I2C_MCS_ACK |                 // positive data ack
                I2C_MCS_RUN);                 // master enable
  while (I2C1_MCS_R & I2C_MCS_BUSY) {};       // wait for transmission done
  data[2] = I2C1_MDR_R;                       // read byte
// fourth
	I2C1_MCS_R = (I2C_MCS_ACK |                 // positive data ack
                I2C_MCS_RUN);                 // master enable
  while (I2C1_MCS_R & I2C_MCS_BUSY) {};       // wait for transmission done
  data[3] = I2C1_MDR_R;                       // read byte
// fifth
	I2C1_MCS_R = (I2C_MCS_ACK |                 // positive data ack
                I2C_MCS_RUN);                 // master enable
  while (I2C1_MCS_R & I2C_MCS_BUSY) {};       // wait for transmission done
  data[4] = I2C1_MDR_R;                       // read byte
// sixth     
  I2C1_MCS_R = (I2C_MCS_STOP |                // generate stop
                I2C_MCS_RUN);                 // master enable
  while (I2C1_MCS_R & I2C_MCS_BUSY) {};       // wait for transmission done
  data[5] = I2C1_MDR_R;                       // least significant byte
  return (I2C1_MCS_R & (I2C_MCS_DATACK|I2C_MCS_ADRACK|I2C_MCS_ERROR));
}
